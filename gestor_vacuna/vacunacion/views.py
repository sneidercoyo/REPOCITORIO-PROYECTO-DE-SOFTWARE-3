...
# Redirigir a la vista de administración solo si el usuario es staff (médico/veterinario)
@login_required
@user_passes_test(lambda user: user.is_staff)  # Verifica si el usuario tiene permisos de staff
def login_medicos(request):
    return redirect('/admin/')  # Redirige al panel de administración

# Login para veterinarios
def login_veterinarios(request):
    return render(request, 'vacunacion/login_veterinarios.html')

# Login para usuarios
def login_usuarios(request):
    return render(request, 'vacunacion/login_usuarios.html')
